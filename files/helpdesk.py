...
# === Helper to read & clean markdown/html docs ===
...
async def handle_message(update: Update, context: CallbackContext):
    ...
    # Step 2: Check if result is useful
    if relevant_docs and any(len(doc.page_content.strip()) > 5 for doc in relevant_docs):
        ...
        print("context chroma :", context_text)
    else:
        logger.info("No match in ChromaDB — falling back to markdown/html docs")
...
# === Bot App Runner ===
def main():
    app = Application.builder().token("7400853171:AAFM5DVXu1w9Jx_mN3xgbP5SHfWFr3cK3kk").build()
